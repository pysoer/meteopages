...
export function createEvaporationPan(): THREE.Group {
  ...
  const boxMat = new THREE.MeshStandardMaterial({ color: 0xf2f3f1, roughness: 0.5, metalness: 0.05 })
  ...
  // ── 7. Surface：草地旁白色平顶控制箱 + 小型雨量器 ──
  const cabinet = new THREE.Group()
  const cbBody = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.22, 0.7), boxMat)
  cbBody.position.y = 0.11
  cabinet.add(cbBody)
  const cbLid = new THREE.Mesh(new THREE.BoxGeometry(0.98, 0.05, 0.76), boxMat)
  cbLid.position.y = 0.245
  cabinet.add(cbLid)
  cabinet.position.set(OUTER_R + 1.25, 0, 0.7)
  root.add(cabinet)

  const rainG = new THREE.Group()
  const rc = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.32, 18), steelMat)
  rc.position.y = 0.16
  rainG.add(rc)
  const rfunnel = new THREE.Mesh(new THREE.CylinderGeometry(0.11, 0.09, 0.06, 18), steelMat)
  rfunnel.position.y = 0.35
  rainG.add(rfunnel)
  rainG.position.set(OUTER_R + 1.2, 0, -0.5)
  root.add(rainG)
